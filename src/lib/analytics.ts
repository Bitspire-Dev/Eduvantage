import type { ConsentState } from './cookies';

export const GTM_ID = 'GTM-KFWJ8VBP';
export const GA_ID = 'G-4GDRGVGJCN';

export function loadGtmScript(): string {
  return `(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start': new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','${GTM_ID}');`;
}

export function loadGaScript(): string {
  return `window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', '${GA_ID}');`;
}

export function shouldLoadAnalytics(consent: ConsentState | null): boolean {
  return !!consent?.analytics;
}
